...
public class MyLevel extends Level{
	...
	public void create(MyDNA dna, int type)
	{
        ...
        for (int i = 0; i < dna.getChromosome().length(); i++) {
            int curr = Character.getNumericValue(dna.getChromosome().charAt(i));
            int startingX = i * 10;
		    if (curr == 0) {
		        buildCoinsAndGoombas(startingX);
            } else if (curr == 1) {
                buildCoinsAndPlatforms(startingX);
            } else if (curr == 2) {
                buildCoinsAndJumps(startingX);
            } else if (curr == 3) {
		        buildJumpsAndGoombas(startingX);
            } else if (curr == 4) {
                buildJumpsAndPlatforms(startingX);
            } else if (curr == 5) {
                buildJumpsAndGoombas(startingX);
            } else if (curr == 6) {
                buildGoombasAndPlatforms(startingX);
            } else {
		         System.out.println("No mapping for this chromosome input.");
            }
        }

//        buildGoombasAndPlatforms(STARTOFFSET);
	...
	public int buildCoins(int x0) {
        for (int x = x0; x < x0 + 10; x++) {
            setBlock(x, DEFAULTHEIGHT-2, HILL_TOP);
            setBlock(x, DEFAULTHEIGHT-1, GROUND);
            for (int y = DEFAULTHEIGHT - 10; y < DEFAULTHEIGHT - 2; y++) {
                setBlock(x, y, COIN);
            }
        }

//        for (int x = x0 + 1; x < x0 + 10; x++) {
//            setBlock(x, DEFAULTHEIGHT - 6, BLOCK_COIN);
//        }
        return x0 + 10;

    }

    public int buildJumps(int x0) {
        for (int x = x0; x < x0 + 10; x++) {
            if (x % 2 == 0) {
                setBlock(x, DEFAULTHEIGHT - 2, HILL_TOP);
                setBlock(x, DEFAULTHEIGHT - 1, GROUND);
    ...
    public int buildGoombas(int x0) {
        for (int x = x0; x < x0 + 10; x++) {
            setBlock(x, DEFAULTHEIGHT-2, HILL_TOP);
            setBlock(x, DEFAULTHEIGHT-1, GROUND);
            setSpriteTemplate(x, DEFAULTHEIGHT - 3, new SpriteTemplate(Enemy.ENEMY_GOOMBA, false));
//            setSpriteTemplate(x, DEFAULTHEIGHT - 5, new SpriteTemplate(Enemy.ENEMY_GOOMBA, true));

        }
        return x0 + 10;
    }

        public int buildPlatforms(int x0) {
        for (int x = x0; x < x0 + 10; x++) {
            setBlock(x, DEFAULTHEIGHT-2, HILL_TOP);
            setBlock(x, DEFAULTHEIGHT-1, GROUND);

            setBlock(x, 8, ROCK);
            setBlock(x, DEFAULTHEIGHT-5, ROCK);
...
//            setSpriteTemplate(x, DEFAULTHEIGHT - 5, new SpriteTemplate(Enemy.ENEMY_GOOMBA, true));
    ...
    public int buildCoinsAndJumps(int x0) {
        for (int x = x0; x < x0 + 10; x++) {
            if (x % 2 == 0) {
                setBlock(x, DEFAULTHEIGHT-2, HILL_TOP);
                setBlock(x, DEFAULTHEIGHT-1, GROUND);
    ...
    public int buildCoinsAndGoombas(int x0) {
        for (int x = x0; x < x0 + 10; x++) {
            this.setBlock(x, DEFAULTHEIGHT-2, HILL_TOP);
            this.setBlock(x, DEFAULTHEIGHT-1, GROUND);
            for (int y = DEFAULTHEIGHT - 10; y < DEFAULTHEIGHT - 2; y++) {
            ...
            setSpriteTemplate(x, DEFAULTHEIGHT - 3, new SpriteTemplate(Enemy.ENEMY_GOOMBA, true));
        }
        return x0 + 10;
    }

    public int buildCoinsAndPlatforms(int x0) {
        for (int x = x0; x < x0 + 10; x++) {
            this.setBlock(x, DEFAULTHEIGHT-2, HILL_TOP);
            this.setBlock(x, DEFAULTHEIGHT-1, GROUND);
            for (int y = DEFAULTHEIGHT - 10; y < DEFAULTHEIGHT - 2; y++) {
    ...
    public int buildJumpsAndGoombas(int x0) {
        for (int x = x0; x < x0 + 10; x++) {
            if (x % 2 == 0) {
                setBlock(x, DEFAULTHEIGHT-2, HILL_TOP);
                setBlock(x, DEFAULTHEIGHT-1, GROUND);
    ...
    public int buildJumpsAndPlatforms(int x0) {
        for (int x = x0; x < x0 + 10; x++) {
            if (x % 2 == 0) {
                setBlock(x, DEFAULTHEIGHT-2, HILL_TOP);
                setBlock(x, DEFAULTHEIGHT-1, GROUND);
    ...
    public int buildGoombasAndPlatforms(int x0) {
        for (int x = x0; x < x0 + 10; x++) {
            setBlock(x, DEFAULTHEIGHT - 2, HILL_TOP);
            setBlock(x, DEFAULTHEIGHT - 1, GROUND);
