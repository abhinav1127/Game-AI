...
//		MyDNA dna = new MyDNA();
	...
	private MyDNA generateRandomIndividual ()
	{
		...
		for (int i = 0; i < 20; i++) {
			chromosome = (random.nextInt(6)) + chromosome;
		}
	...
	private boolean terminate (ArrayList<MyDNA> population, int count)
	{
		...
		// YOUR CODE GOES BELOW HERE
		for (MyDNA member: population) {
			if (member.getFitness() >= 0.8) {
				decision = true;
			}
		}
	...
	private ArrayList<MyDNA> selectIndividualsForMutation (ArrayList<MyDNA> population)
	{
		...
		for (int i = population.size() - 1; i > population.size() - 11; i--) {
	...
	{
		int num = 20; // Default needs to be changed
		// YOUR CODE GOES BELOW HERE
	...
	{
		int num = 5; // Default is no crossovers
		// YOUR CODE GOES BELOW HERE
	...
	private MyDNA pickIndividualForCrossover (ArrayList<MyDNA> population, MyDNA excludeMe)
	{
		...
		// YOUR CODE GOES BELOW HERE
		Collections.sort(population);
		if (population.get(population.size() - 1) != excludeMe) {
			picked = population.get(population.size() - 1);
		} else {
			picked = population.get(population.size() - 2);
		}
	...
	private ArrayList<MyDNA> globalCompetition (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation)
	{
		...
		int a = oldPopulation.size() - 1;
		int b = newPopulation.size() - 1;

		for (int i = 0; i < 20; i++) {
			if (newPopulation.get(b).getFitness() > oldPopulation.get(a).getFitness()) {
				finalPopulation.add(newPopulation.get(b));
				b--;
			} else {
				finalPopulation.add(oldPopulation.get(a));
				a--;
			}
		}
