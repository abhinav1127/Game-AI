...
class Agent:
	...
	def updateVtable(self, newState, lastState, action, reward, terminal, availableActions):
		...
		origValue = self.v_table[lastState][action]

		if terminal:
			origValue += (self.learningRate * (reward - origValue))

		else:
			bestAction = self.greedy(newState)
			bestReward = self.v_table[newState][bestAction]

			origValue += (self.learningRate * (reward + (self.gamma * bestReward) - origValue))

		self.v_table[lastState][action] = origValue
	...
	def greedy(self, observation):
		if type(observation) != tuple:
			...
			curr = self.v_table[tuple(observation.worldState)]

		else:
			self.initializeVtableStateEntry(observation)
			curr = self.v_table[observation]


		maxScore = curr[0]
		maxIndex = 0

		for (index, score) in enumerate(curr):
			if score > maxScore:
				maxScore = score
				maxIndex = index
